    ...
    const sketch = (p) => {
      ...
      p.draw = () => {
        ...
        if (Object.keys(currentEmbeddings).length > 0 && currentWords.length > 0) {
          ...
          p.noStroke();
          p.textAlign(p.CENTER, p.CENTER);
          p.text('Click "Spin the Centrifuge!" to see embeddings', p.width/2, p.height/2);
        }
    ...
    const sketch = (p) => {
      ...
      p.draw = () => {
        ...
        if (Object.keys(currentSearchEmbeddings || {}).length > 0) {
          ...
          p.noStroke();
          p.textAlign(p.CENTER, p.CENTER);
          p.text('Click "Search the Cupboards!" to see query-document relationships', p.width/2, p.height/2);
        }
